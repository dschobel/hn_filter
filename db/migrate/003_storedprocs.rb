class Storedprocs < ActiveRecord::Migration
  def self.up
    execute "CREATE OR REPLACE FUNCTION update_statistics(timeframe integer) RETURNS VOID as $$
            DECLARE
            mn REAL;
            md REAL;
            minimum INTEGER;
  ...
  def self.down
    execute "DROP FUNCTION update_statistics(integer)"
  end
end
